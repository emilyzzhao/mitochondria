...
plot( fit2.nlsList, id = 0.05, adj = -1 ) 
qqnorm( fit2.nlsList, abline = c(0,1) )

summary(fit2.nlsList)
...
fit3.nlme.withrandom <- update(fit3.nlme, groups = ~pair, random = K~1) 

anova(fit2.nlme, fit3.nlme)
...
fit2.nlsList_GM <- nlsList(y ~ SSmicmen(dose, Vm, K)|geno, data = dPsi_GM)

fit2.nlme_GM <- nlme(fit2.nlsList_GM)

fit2.nlme_GM <- update(fit2.nlme_GM, fixed = list(Vm ~ 1, K ~ 1), groups = ~geno/pair, random = (K~1|geno/pair)) 
summary(fit2.nlme_GM)


fit_reduced <- nlme(fit2.nlsList_GM, groups = ~pair, random = K ~ 1, fixed = y ~ SSmicmen(dose, Vm, K))
summary(fit_reduced)


lrt <-anova(fit2.nlme_GM, fit_reduced)
